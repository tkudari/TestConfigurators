 ...
 */
	public static boolean checkResult(String testCasesFile, String valueToRead, Context context, Solo solo){
		...
		String expectedUIMessage = DWall_intg_test_helper.getConfigProperty(testCasesFile, valueToRead, context);
		...
		if (result)
			...
			Log.e(TAG,"Found text: " + solo.getText(1).getText());
			Log.e(TAG,"Found text: " + solo.getText(2).getText());
		}	
		return result;
	}
 ...
 */	
	public static String createFileFromBitMap(Bitmap bitmap, String fileName){
		Log.d(TAG,"createFileFromBitMap(). External Storage Writable ? / Readable ? " + isExternalStorageWritable() + isExternalStorageReadable());
		
		final String screenShotsPath = Environment.getExternalStorageDirectory() + File.separator + "Pictures"+ File.separator;
		String fullfileName = screenShotsPath + fileName + "_" + System.currentTimeMillis() + ".jpg";
