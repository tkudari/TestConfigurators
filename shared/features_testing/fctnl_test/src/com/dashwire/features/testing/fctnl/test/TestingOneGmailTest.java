...
import com.dashwire.features.testing.fctnl.test.mock.FeatureConfigDataMocks;
import com.dashwire.testing.helper.controller.TestingHelperService;

public class TestingOneGmailTest extends AndroidTestCase {
	...
	@UiThreadTest
	public void testOnePositiveGmail() {
		String featureAssetValue = "one_positive_gmail";
		String featureName = "Account";
		String screenToTest = Intent.ACTION_MAIN;
		JSONArray featureConfigs = FeatureConfigDataMocks.loadFeatureConfigDataMocks(getTestContext(), featureAssetValue);
		Log.v(TAG,"featureConfigs = " + featureConfigs.toString());
		testingFeatures(featureAssetValue, featureName, screenToTest, featureConfigs, "Positive");
	}
	
	@UiThreadTest
	public void testOneNegativeGmail() {
		String featureAssetValue = "one_negative_gmail";
		String featureName = "Account";
		String screenToTest = Intent.ACTION_MAIN;
		JSONArray featureConfigs = FeatureConfigDataMocks.loadFeatureConfigDataMocks(getTestContext(), featureAssetValue);
		Log.v(TAG,"featureConfigs = " + featureConfigs.toString());
	...
	public void testingFeatures(String featureAssetValue, String featureName, String screenToTest, JSONArray featureConfigs, String testType) {
		...
		try {
			if (featureConfigs != null && isFeatureControllerReady()) {
				for (int i = 0; i < totalFeaturesToTest; i++) {
					...
					broadcastFeatureToTest(featureId, featureName, featureData, screenToTest);
				}

				Log.v(TAG, "Waiting Till features get configured");
				waitTillFeaturesConfigured();
				...
				// assertTrue(totalFeaturesToTest == totalFeaturesTested);
				...
				featuresTestingControllerService.reset();
			} else
			{
	...
	public void waitTillFeaturesConfigured() {
		if ("COMPLETED".equalsIgnoreCase(featuresTestingControllerService.getFeatureControllerStatus())) {
			return;
		} else {
			Log.v(TAG, "********Featrues are not yet configured*********");
			try {
				Thread.sleep(2000);
	...
	private boolean isFeatureControllerReady() {
		if ("INIT".equalsIgnoreCase(featuresTestingControllerService.getFeatureControllerStatus())) {
			return true;
		} else {
			Log.e(TAG, "********FeaturesControllerService is not Ready yet*******");
			try {
				Thread.sleep(2000);
			} catch (InterruptedException e) {
			}
			isFeatureControllerReady();
		}
		return false;
	}

	private void broadcastFeatureToTest(String featureId, String featureName, String featureData, String screenToTest) {
		Intent featureConfiguratorIntent = new Intent("com.dashwire.features.testing.fctnl.test");
		...
		featureConfiguratorIntent.putExtra("screenToTest", screenToTest);
		context.sendBroadcast(featureConfiguratorIntent);
	}
